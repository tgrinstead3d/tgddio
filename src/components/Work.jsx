...
const Work = ({ theme, onOpenCaseStudy }) => {
  ...
  // Separate Hero project (Smalls and Petite) from others
  const heroProject = featuredProjects.find(p => p.title === "Smalls and Petite");
  const secondaryProjects = featuredProjects.filter(p => p.title !== "Smalls and Petite");
  ...
  return (
    <section id="work" className={`py-16 md:py-24 ${theme.cardBgAlt} transition-colors duration-500`}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        {heroProject && (
          <div className="mb-8">
            <div className="group relative rounded-2xl overflow-hidden shadow-lg block h-[500px]">
              {/* Project Image - Clickable */}
              {heroProject.hasCaseStudy ? (
                <button 
                  onClick={() => onOpenCaseStudy(heroProject)}
                  className="absolute inset-0 cursor-pointer z-10 w-full h-full text-left"
                  aria-label={`View Case Study for ${heroProject.title}`}
                >
                  <img
                ...
                <a 
                  ...
                  target={heroProject.link === '#' ? '_self' : '_blank'}
                  rel={heroProject.link === '#' ? '' : 'noopener noreferrer'}
                  onClick={(e) => heroProject.link === '#' && e.preventDefault()}
              ...
              <div className="relative h-full p-8 md:p-12 flex flex-col justify-end z-20 pointer-events-none">
                <div className={`transform transition-all duration-500 translate-y-4 group-hover:translate-y-0`}>
                  ...
                  <h4 className="text-4xl md:text-5xl font-bold text-white mb-4">{heroProject.title}</h4>
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {secondaryProjects.map((project, idx) => (
            <div 
              key={idx} 
              className={`group relative rounded-2xl overflow-hidden shadow-lg block h-96`}
            >
              {/* Project Image - Clickable */}
              ...
              {project.link === '#' ? (
                <div className="relative h-full p-6 flex items-center justify-center z-20 pointer-events-none">
                  <div className="text-2xl font-bold text-white uppercase tracking-widest border-4 border-white/20 p-4 rounded-xl backdrop-blur-sm">
                    Coming Soon
                  </div>
                </div>
              ) : (
                <div className="relative h-full p-6 flex flex-col justify-end z-20 pointer-events-none">
                  <div className={`transform transition-all duration-500 translate-y-4 group-hover:translate-y-0`}>
                    <div className="inline-block px-2 py-1 bg-white/20 backdrop-blur-md rounded-full text-white text-[10px] font-bold uppercase tracking-wider mb-2 border border-white/30">
                      {project.category}
                    </div>
                    <h4 className="text-xl font-bold text-white mb-2">{project.title}</h4>
                    <p className="text-white/90 text-sm opacity-0 transform translate-y-4 transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0 delay-100 line-clamp-3">
                      {project.desc}
                    </p>

                    <div className="mt-4 flex flex-col gap-3 opacity-0 transition-opacity duration-500 group-hover:opacity-100 delay-200 pointer-events-auto">
                      <a 
                        href={project.link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 text-white text-sm font-bold hover:text-cyan-400 transition-colors"
                      >
                      ...
                      {project.hasCaseStudy && (
                        <button
                          onClick={() => onOpenCaseStudy(project)}
                          className="px-3 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 rounded-lg text-white text-xs font-bold transition-all cursor-pointer w-fit"
                        >
                          View Case Study
                        </button>
                      )}
